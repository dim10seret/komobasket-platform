...
describe("KomoControl local immutable GamePackage store", () => {
    it("migrates an existing 0001 database through 0003", () => { const f = fixture({ includeV2: false }); createLegacyDatabase(f); fs.copyFileSync(path.resolve("electron/migrations/0002_game_packages.sql"), path.join(f.migrationsDirectory, "0002_game_packages.sql")); fs.copyFileSync(path.resolve("electron/migrations/0003_game_runs.sql"), path.join(f.migrationsDirectory, "0003_game_runs.sql")); const reopened = new LocalDatabase({ databasePath: f.localDatabase.databasePath, migrationsDirectory: f.migrationsDirectory, backupDirectory: path.join(f.root, "backups") }); databases.push(reopened); expect(reopened.initialize().schemaVersion).toBe("0003_game_runs.sql"); });
