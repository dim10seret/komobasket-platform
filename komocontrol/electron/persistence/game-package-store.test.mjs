...
describe("KomoControl local immutable GamePackage store", () => {
    it("migrates an existing 0001 database through 0004", () => { const f = fixture({ includeV2: false }); createLegacyDatabase(f); for (const migration of ["0002_game_packages.sql", "0003_game_runs.sql", "0004_game_run_configuration.sql"]) fs.copyFileSync(path.resolve("electron/migrations", migration), path.join(f.migrationsDirectory, migration)); const reopened = new LocalDatabase({ databasePath: f.localDatabase.databasePath, migrationsDirectory: f.migrationsDirectory, backupDirectory: path.join(f.root, "backups") }); databases.push(reopened); expect(reopened.initialize().schemaVersion).toBe("0004_game_run_configuration.sql"); });
