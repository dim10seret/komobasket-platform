...
function fixture(options = {}) {
    ...
    if (options.includeV2 !== false) {
        fs.copyFileSync(path.resolve("electron/migrations/0002_game_packages.sql"), path.join(migrationsDirectory, "0002_game_packages.sql"));
        fs.copyFileSync(path.resolve("electron/migrations/0003_game_runs.sql"), path.join(migrationsDirectory, "0003_game_runs.sql"));
...
describe("KomoControl local immutable GamePackage store", () => {
    it("migrates an existing 0001 database through 0008", () => { const f = fixture({ includeV2: false }); createLegacyDatabase(f); for (const migration of ["0002_game_packages.sql", "0003_game_runs.sql", "0004_game_run_configuration.sql", "0005_match_gameplay.sql", "0006_gameplay_sync.sql", "0007_live_pre_game_corrections.sql", "0008_resumable_live_flows.sql"]) fs.copyFileSync(path.resolve("electron/migrations", migration), path.join(f.migrationsDirectory, migration)); const reopened = new LocalDatabase({ databasePath: f.localDatabase.databasePath, migrationsDirectory: f.migrationsDirectory, backupDirectory: path.join(f.root, "backups") }); databases.push(reopened); expect(reopened.initialize().schemaVersion).toBe("0008_resumable_live_flows.sql"); });
    it("preserves device identity across migration", () => { const f = fixture({ includeV2: false }); const before = createLegacyDatabase(f); for (const migration of ["0002_game_packages.sql", "0003_game_runs.sql", "0004_game_run_configuration.sql", "0005_match_gameplay.sql", "0006_gameplay_sync.sql", "0007_live_pre_game_corrections.sql", "0008_resumable_live_flows.sql"]) fs.copyFileSync(path.resolve("electron/migrations", migration), path.join(f.migrationsDirectory, migration)); const reopened = new LocalDatabase({ databasePath: f.localDatabase.databasePath, migrationsDirectory: f.migrationsDirectory, backupDirectory: path.join(f.root, "backups") }); databases.push(reopened); expect(reopened.initialize().deviceIdentity.deviceId).toBe(before); });
