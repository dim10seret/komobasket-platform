...
describe("KomoControl local persistence", () => {
    it("creates migrations 0001 through 0004, their checksums, device identity, durability settings and healthy schema", () => {
        ...
        try {
            ...
            const expectedMigrations = ["0001_initial.sql", "0002_game_packages.sql", "0003_game_runs.sql", "0004_game_run_configuration.sql", "0005_match_gameplay.sql", "0006_gameplay_sync.sql", "0007_live_pre_game_corrections.sql"].map((migrationId) => ({
                migration_id: migrationId,
                checksum: createHash("sha256").update(fs.readFileSync(path.join(migrationsDirectory, migrationId))).digest("hex"),
            }));

            expect(status.schemaVersion).toBe("0007_live_pre_game_corrections.sql");
            expect(status.deviceIdentity.deviceId).toMatch(/^[0-9a-f-]{36}$/);
            expect(migrations).toEqual(expectedMigrations);
            expect(tables).toEqual(["device_identity", "local_game_packages", "local_game_run_configurations", "local_game_runs", "local_gameplay_sync_state", "local_live_run_authorizations", "local_match_engine_snapshots", "local_match_events", "local_match_finalizations", "local_schema_migrations"]);
    ...
    it("does not reapply migrations and preserves one device identity after reopen", () => {
        ...
        expect(db.prepare("SELECT COUNT(*) AS count FROM local_schema_migrations").get().count).toBe(7);
        db.close();
    });
    ...
    it("creates and independently verifies an online backup with the same device identity", async () => {
        ...
        expect(result.schemaVersion).toBe("0007_live_pre_game_corrections.sql");
