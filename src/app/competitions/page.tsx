...
function CompactGameRow({ game }: { game: PublicGame }) {
  const homeWinner = game.homeScore !== null && game.awayScore !== null && game.homeScore > game.awayScore;
  const awayWinner = game.homeScore !== null && game.awayScore !== null && game.awayScore > game.homeScore;
  const score = game.homeScore !== null && game.awayScore !== null ? `${game.homeScore} – ${game.awayScore}` : "vs";
  return <article className="grid min-h-12 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 rounded-xl border border-zinc-200 bg-white px-3 py-2 shadow-sm"><span className={`min-w-0 truncate text-sm ${homeWinner ? "font-black text-zinc-950" : "font-bold text-zinc-700"}`}>{game.homeTeam.name}</span><span className={`rounded-lg px-2.5 py-1 text-sm tabular-nums ${game.homeScore !== null && game.awayScore !== null ? "bg-zinc-950 font-black text-white" : "bg-orange-50 font-bold text-orange-800"}`}>{score}</span><span className={`min-w-0 truncate text-right text-sm ${awayWinner ? "font-black text-zinc-950" : "font-bold text-zinc-700"}`}>{game.awayTeam.name}</span></article>;
}

function GameCard({ game, compact = false }: { game: PublicGame; compact?: boolean }) {
  if (compact) return <CompactGameRow game={game} />;
  ...
  return <article className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm"><div className={`grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2.5 md:gap-4 ${hasMetadata ? "md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_minmax(11rem,auto)]" : "md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]"}`}><Team game={game} side="home" /><div className={`min-w-14 rounded-xl px-2.5 py-1.5 text-center text-base tabular-nums sm:min-w-16 sm:text-lg ${hasResult ? "bg-zinc-950 font-black text-white" : "bg-orange-50 font-bold text-orange-800"}`}>{hasResult ? `${game.homeScore} – ${game.awayScore}` : "vs"}</div><Team game={game} side="away" />{hasMetadata && <div className="col-span-3 mt-1 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 border-t border-zinc-100 pt-2 text-xs font-bold uppercase tracking-wide text-zinc-500 md:col-span-1 md:col-start-4 md:row-start-1 md:mt-0 md:justify-start md:border-0 md:pt-0">{details.length > 0 && <span>{details.join(" · ")}</span>}{game.venue && <VenueDetails venue={game.venue} />}{game.videoUrl && <a href={game.videoUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-orange-100 px-2.5 py-1 text-orange-800 hover:bg-orange-200">▶ Βίντεο</a>}</div>}</div></article>;
}
