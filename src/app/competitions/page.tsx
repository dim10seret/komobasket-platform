...
function CompactGameRow({ game, teamHref }: { game: PublicGame; teamHref?: (teamId: string) => string }) {
  ...
  return <article className="grid min-h-12 grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 rounded-xl border border-zinc-200 bg-white px-3 py-2 shadow-sm"><Team game={game} side="home" compact teamHref={teamHref} /><PublicGameResult game={game} className={`rounded-lg px-2.5 py-1 text-sm tabular-nums ${hasResult ? "bg-zinc-950 font-black text-white" : "bg-orange-50 font-bold text-orange-800"}`} /><Team game={game} side="away" compact teamHref={teamHref} /></article>;
}
