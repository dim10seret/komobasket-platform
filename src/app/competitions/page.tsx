...
function GameCard({ game, compact = false, teamHref }: { game: PublicGame; compact?: boolean; teamHref?: (teamId: string) => string }) {
  ...
  return <article className="rounded-2xl border border-zinc-200 bg-white p-3 shadow-sm"><div className={`grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2.5 md:gap-4 ${hasMetadata ? "md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_minmax(11rem,auto)]" : "md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]"}`}><Team game={game} side="home" teamHref={teamHref} /><div className={`min-w-14 rounded-xl px-2.5 py-1.5 text-center text-base tabular-nums sm:min-w-16 sm:text-lg ${hasResult ? "bg-zinc-950 font-black text-white" : "bg-orange-50 font-bold text-orange-800"}`}>{hasResult ? `${game.homeScore} – ${game.awayScore}` : "vs"}</div><Team game={game} side="away" teamHref={teamHref} />{hasMetadata && <div className="col-span-3 mt-1 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 border-t border-zinc-100 pt-2 text-xs font-bold uppercase tracking-wide text-zinc-500 md:col-span-1 md:col-start-4 md:row-start-1 md:mt-0 md:justify-start md:border-0 md:pt-0">{details.length > 0 && <span>{details.join(" · ")}</span>}{game.venue && <VenueDetails venue={game.venue} />}{game.videoUrl && <a href={game.videoUrl} target="_blank" rel="noopener noreferrer" className="rounded-full bg-orange-100 px-2.5 py-1 text-orange-800 hover:bg-orange-200">▶ Βίντεο</a>}</div>}</div></article>;
}
