...
const mocks = vi.hoisted(() => ({ requireAdmin: vi.fn(), resolveUser: vi.fn(), requireAccess: vi.fn(), authorizationError: vi.fn(), readSheet: vi.fn(), generate: vi.fn() }));
...
describe("protected printable Game Sheet route", () => {
  ...
  it("returns read-only printable PDF", async () => { const response = await call(); expect(response.status).toBe(200); expect(response.headers.get("Content-Type")).toBe("application/pdf"); expect(response.headers.get("Cache-Control")).toBe("private, no-store"); expect(response.headers.get("Content-Disposition")).toContain("komobasket-game-sheet.pdf"); });
  it("fails closed for unavailable finalized data", async () => { mocks.readSheet.mockResolvedValue({ kind: "unavailable", reason: "MATCH_REPORT_UNAVAILABLE" }); expect((await call()).status).toBe(409); expect(mocks.generate).not.toHaveBeenCalled(); });
