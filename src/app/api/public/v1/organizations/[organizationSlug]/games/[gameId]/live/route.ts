...
export async function GET(request: Request, context: { params: Promise<{ organizationSlug: string; gameId: string }> }) {
  const { organizationSlug, gameId } = await context.params;
  const organization = await resolveHostedPublicOrganization(organizationSlug);
  try {
    const result = await readPublicLiveGameForOrganization(organization.organizationId, gameId, { ifNoneMatch: request.headers.get("if-none-match") });
