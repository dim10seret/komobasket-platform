...
function database() {
  const calls = [];
  return { calls, binding: { prepare(query) { let values = []; const statement = { bind(...next) { values = next; calls.push({ query, values }); return statement; }, async first() { return null; }, async run() { return {}; }, async all() { if (query.includes("FROM league_competitions")) { const prefix = values[0] === "organization_runbasket" ? "runbasket" : "komobasket"; return { results: [{ season_id: `season_${prefix}`, season_name: "2026-27", season_slug: "2026-27", starts_on: "2026-08-01", competition_id: `competition_${prefix}`, competition_name: `${prefix} competition`, competition_slug: `${prefix}-competition` }] }; } if (query.includes("FROM league_games")) return { results: [{ game_id: values[0].replace("competition_", "game_"), round_number: 1, round_label: "1η Αγωνιστική" }] }; return { results: [] }; } }; return statement; }, async batch() { return []; } } };
}
