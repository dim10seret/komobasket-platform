...
export function platformLeagueActions(actor: CanonicalAppUser, scopeOrganizationId?: string) {
 const { requireCompetitionAccess, requireCompetitionVenueAccess, requireGameAccess, requireOrganizationAccess, requirePhaseDependencyGraphAccess, requirePlayerAccess, requireRosterMembershipAccess, requireRosterRelationshipAccess, requireStaffAccess, requireStaffMembershipAccess, requireStaffRosterRelationshipAccess, requireTeamCompetitionAccess, requireTransferRelationshipAccess } = platformOperationScope(scopeOrganizationId);
 const resolveActor = async (identity: CanonicalAppUser) => identity;
async function PATCH(request: Request) {
  try {
    const input = (await request.json()) as Record<string, unknown>;
    const action = String(input.action ?? "");
    let canonicalUserPromise: ReturnType<typeof resolveActor> | null = null;
    const canonicalUser = () => {
      canonicalUserPromise ??= resolveActor(actor);
    ...
    if (action === "searchAthletes") {
      const canonicalUser = await resolveActor(actor);
    ...
    if (action === "createRegistryPlayer") {
      const requestedOrganizationId = String(input.organizationId ?? input.organization_id ?? "").trim();
      const organization = await requireOrganizationAccess(await canonicalUser(), requestedOrganizationId, "manage");
      return Response.json(await createAthleteCanonical({
        firstName: String(input.firstName ?? ""),
        lastName: String(input.lastName ?? ""),
        birthDate: input.birthDate ? String(input.birthDate) : null,
        photoUrl: input.photoUrl ? String(input.photoUrl) : null,
    ...
    if (action === "addAthleteMovement") {
      await requireRosterRelationshipAccess(await canonicalUser(), {
        playerId: String(input.playerId ?? ""),
        competitionId: String(input.competitionId ?? ""),
        teamId: String(input.teamId ?? ""),
      }, "manage");
      return Response.json(await addAthleteToCompetitionRosterWithMovement({
        playerId: String(input.playerId ?? ""),
        seasonId: String(input.seasonId ?? ""),
        competitionId: String(input.competitionId ?? ""),
        teamId: String(input.teamId ?? ""),
    ...
    if (action === "finalizePhase") {
      ...
      return Response.json(await finalizeLeaguePhase({ phaseId, competitionId }, actor.email));
    }
    if (action === "departPlayerLegacy") {
      await requireRosterMembershipAccess(await canonicalUser(), String(input.rosterId ?? ""), "manage");
      return Response.json(await departPlayer(input, actor.email));
    }
