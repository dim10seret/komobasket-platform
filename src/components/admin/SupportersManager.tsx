...
export default function SupportersManager() {
  ...
  async function toggle(supporter: Supporter) {
    ...
    try {
      const response = await fetch("/api/admin/supporters", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: supporter.id, status: supporter.status === "active" ? "inactive" : "active" }),
      });
      const payload = await response.json();
  ...
  async function remove(supporter: Supporter) {
    ...
    try {
      const response = await fetch("/api/admin/supporters", {
        method: "DELETE",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ id: supporter.id }),
      });
      const payload = await response.json();
  ...
  return <main className="mx-auto max-w-[1200px] px-4 py-8 sm:px-7">
    <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
      <div>
        <p className="text-xs font-black uppercase tracking-[.22em] text-orange-600">KomoBasket CMS</p>
        <h2 className="mt-2 text-3xl font-black text-zinc-950">Υποστηρικτές &amp; Συνεργάτες</h2>
        <p className="mt-2 text-zinc-600">Κεντρικό περιεχόμενο KomoBasket.</p>
      </div>
      <button type="button" className={buttonClass} onClick={() => { resetForm(); setNotice(""); }}>+ Νέος Υποστηρικτής</button>
    </div>
    ...
    <form onSubmit={save} className="mb-8 grid gap-4 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:grid-cols-2">
