...
export default function NewsManager() {
  ...
  async function submitArticle(
    ...
    try {
      ...
      if (!response.ok || !data.article) {
        throw new Error(
          data.error || "Η ανακοίνωση δεν αποθηκεύτηκε.",
        );
      }
  ...
  function addPendingAttachments(
    ...
    const selected = Array.from(files).map(
      (file) => ({
    ...
    if (
      ...
      setError(
        "Κάθε ανακοίνωση μπορεί να έχει έως 10 συνημμένα.",
      );
  ...
  async function renameAttachment(
    ...
    setMessage(
      "Το όνομα του συνημμένου ενημερώθηκε.",
    );
  ...
  async function moveAttachment(
    ...
    const article = articles.find(
      (candidate) =>
        candidate.id === attachment.articleId,
    );
  ...
  async function removeAttachment(
    ...
    if (
      !window.confirm(
        `Να διαγραφεί το συνημμένο «${attachment.name}»;`,
      )
    ) {
      return;
    }
        ...
        <div className="grid gap-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <label className="grid gap-2 font-bold text-zinc-800">
              ...
              >
                <option value="draft">
                  Πρόχειρο — δεν φαίνεται στο
                  site
                </option>

                <option value="published">
                  Δημοσιευμένο — εμφανίζεται
                  στα Νέα
                </option>
              </select>
            </label>
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm font-normal leading-6 text-zinc-500">
                  PDF, Word ή Excel, έως 15
                  MB το καθένα και έως 10
                  αρχεία.
                </p>
              </div>
