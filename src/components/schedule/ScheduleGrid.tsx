...
export default function ScheduleGrid({ archiveSeason }: { archiveSeason?: string } = {}) {
  ...
  function changeSeason(value:string){setSelectedSeason(value); setCompetition("league"); setPhase("1");}
  function changeCompetition(value:string){setCompetition(value); setPhase(value === "league" ? "1" : cupStages[0]);}
  if (season !== "2025-26" && season !== "2024-25") return <section className="bg-zinc-100 py-10"><div className="mx-auto max-w-7xl px-6"><p className="rounded-2xl border border-amber-200 bg-amber-50 p-6 font-bold text-zinc-700">Δεν υπάρχει διαθέσιμο ιστορικό πρόγραμμα για τη σεζόν {season}.</p></div></section>;
  return <section className="bg-zinc-100 pb-16 pt-8"><div className="mx-auto max-w-7xl px-6">
    <div className="mb-8 flex flex-wrap gap-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">{!archiveSeason && <div><label className="mb-2 block text-sm font-bold">Σεζόν</label><select value={season} onChange={(e)=>changeSeason(e.target.value)} className="w-44 rounded-xl border border-zinc-300 px-4 py-3"><option>2025-26</option><option>2024-25</option></select></div>}<div><label className="mb-2 block text-sm font-bold">Διοργάνωση</label><select value={competition} onChange={(e)=>changeCompetition(e.target.value)} className="w-64 rounded-xl border border-zinc-300 px-4 py-3"><option value="league">KomoBasket League</option><option value="cup">{cupName}</option></select></div><div><label className="mb-2 block text-sm font-bold">Φάση</label><select value={phase} onChange={(e)=>setPhase(e.target.value)} className="w-64 rounded-xl border border-zinc-300 px-4 py-3">{competition === "league" ? <>{Array.from({length:15},(_,i)=>i+1).map((round)=><option key={round} value={round}>{round}η Αγωνιστική</option>)}<option>Play out 5-12</option><option>Φάση των 8</option><option>Final4 KomoBasket</option></> : cupStages.map((stage)=><option key={stage}>{stage}</option>)}</select></div></div>
